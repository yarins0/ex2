...
const isGoodMethods = (methods: Sexp[]): methods is [Sexp, Sexp][] =>
    isArray(methods) && allT(isNonEmptyList, methods) && allT(isIdentifier, map(first, methods));

export const parseBinding = (binding: Sexp): Result<Binding> =>
    isNonEmptyList(binding) && binding.length === 2 ? 
    bind(parseL3CExp(second(binding) as string), (val: CExp) => 
         makeOk(makeBinding(first(binding) as string, val))) :
    makeFailure('Malformed binding in "class" expression');
    

export const parseClassExp = (fields: Sexp, methods: Sexp[]): Result<ClassExp> => {
    if (!isGoodFields(fields)) {
        return makeFailure('Malformed fields in "class" expression');
    }
    if (!isGoodMethods(methods)) {
        return makeFailure('Malformed methods in "class" expression');
    }
    const fieldsResult = map(makeVarDecl, fields);
    const methodsResult = mapResult(parseBinding, methods);
    return bind(methodsResult, (methods: Binding[]) => 
                makeOk(makeClassExp(fieldsResult, methods)));
}
...
export const unparseL3 = (exp: Program | Exp): string =>
    ...
    isClassExp(exp) ? `(class (${map((v: VarDecl) => v.var, exp.fields).join(" ")}) (${map((b: Binding) => `(${b.var.var} ${unparseL3(b.val)})`, exp.methods).join(" ")})` :
    exp;
